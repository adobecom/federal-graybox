  ...
  `},xe=({iconHref:n,iconAlt:e,title:r,href:t,subtitle:a,badges:o=[],daaLl:i,daaLh:s})=>{let d=e!==null&&n!==null,c=k(s,i??r),x=d?`
    ...
    `:"",y=o.length===0?"":`
      <div class="feds-link-group__badges">
        ${o.map((p,f)=>`
          <span class="feds-link-group__badge${f>0?" feds-link-group__badge--filled":""}">
            ${p}
          </span>
        `).join("")}
      </div>
    `,g=a===""?"":`<div class="feds-link-group__subtitle">${a}</div>`;return`
    <a class="feds-link-group" href="${t}"${c}>
...
`};var V={elementNull:"Error when parsing Brand. Element is null",noLinks:"Error when parsing Brand. No links found",noPrimaryLink:"Error when parsing Brand. No primary link found"},z=/(\.png|\.jpg|\.jpeg|\.svg)/i,ke=n=>{let e=n.querySelector("picture img")?.getAttribute("src")??null;if(e!==null&&e!=="")return e;let r=n.textContent?.trim();if(r!==void 0&&r!==""&&z.test(r)){let a=r.split("|")[0]?.trim();if(a!==void 0&&a!=="")return a}let t=n.getAttribute("href");return t!==null&&t!==""&&z.test(t)?t:null},Ce=n=>{let e=n.textContent?.trim();if(e?.includes("|")===!0){let t=e.split("|")[1]?.trim();if(t)return t}return n.querySelector("img")?.getAttribute("alt")??""},gn=n=>{if(n===null)throw new l(V.elementNull);let e=n.querySelector(".gnav-brand");if(e===null)throw new l(V.elementNull);let r=[...e.querySelectorAll("a")];if(r.length===0)throw new l(V.noLinks);let t=r.find(m=>{let b=m.textContent??"";return!z.test(m.href)&&!z.test(b)});if(!t)throw new l(V.noPrimaryLink);let a=e.matches(".brand-image-only"),o=e.matches(".no-logo"),i=e.matches(".image-only"),s=!o,d=!a&&!i,c=r.filter(m=>{let b=m.textContent??"";return z.test(m.href)||z.test(b)}),[x,y,g]=(()=>{let m=a?T.brand:T.company,[b=null,w=null]=[...e.querySelectorAll('picture img[src$=".svg"]')].map(tn=>tn?.src).filter(tn=>tn?.length>0),[I=null,W=null]=c.map(ke),se=c[0]instanceof Element?Ce(c[0]):t.textContent?.trim()??"";return[I??b??m,W??w,se]})(),p=t.textContent?.trim()??"",f=t.href;if(!s&&!d)return[{type:"Brand",data:{type:"NoRender"}},[]];let v=(m,b)=>{let w=b!=null&&b!=="";return Pn()&&w?b:m},h=x.startsWith("<svg")?{type:"inline-svg",svgContent:v(x,y),alt:g}:{type:"image",src:v(x,y),alt:g};return s&&d?[{type:"Brand",data:{type:"LabelledBrand",href:f,label:p,image:h}},[]]:s&&a?[{type:"Brand",data:{type:"BrandImageOnly",href:f,image:h,alt:g}},[]]:s&&i?[{type:"Brand",data:{type:"ImageOnlyBrand",href:f,image:h,alt:g}},[]]:[{type:"Brand",data:{type:"BrandLabelOnly",href:f,label:p}},[]]};var fn=(n,e)=>{let r=`feds-brand-image${e?" brand-image-only":""}`;if(n.type==="inline-svg")return`<span class="${r}">${n.svgContent}</span>`;let t=n.alt?` alt="${n.alt}"`:"";return`<span class="${r}"><img src="${n.src}"${t} /></span>`},Le=`
<?xml
...
`,Nn=n=>n.type==="PrimaryCTA"?yn(n):A(n);var G=({text:n,href:e,daaLl:r})=>`<a class="feds-link" href="${e}"${k(null,r??n)}>${n}</a>`;var Dn=n=>{let[e,r]=He(n);return[{type:"LinksCard",card:e},r]},He=n=>{let e=n.querySelector("h2, h3, h4")||null;if(!e)throw new l("Expected links card title");let r=n.querySelector("em > a"),t=[...n.querySelectorAll("a")].filter(d=>d!==r);if(t.length===0)throw new l("Expected at least one link");let[a,o]=M(t,C),[i,s]=(()=>{try{return P(n)}catch{return[null,[]]}})();return i&&(i.daaLl=`${e.textContent??""} - ${i?.daaLl}`),[{type:"LinksCardItem",title:e.textContent??"",links:a,footerCTA:i},[...o,...s]]};var Bn=n=>{let e=[...n.querySelectorAll("li > div")],r=[...n.querySelectorAll("li > a")],[t,a]=M(e,ze),[o,i]=M(r,C);return[{type:"ProductList",categories:t,links:o},[...a,...i]]},ze=n=>{let e=n.firstElementChild;if(e?.nodeName!=="H2")throw new l("Expected H2");let r=e.textContent??"",t=e.textContent??"",a=En(e),[o,i]=M(a,cn);return[{type:"ProductCategory",name:r,daaLh:t,links:o},i]};var On=n=>{let[e,r]=Ue(n);return[{type:"FeaturedCard",card:e},r]},Ue=n=>{let e=n.querySelector("h5")||null;if(!e)throw new u("Eye brow element not found");let r=n.querySelector("h4")||null,t=r?.nextElementSibling||null;if(!r)throw new l("Expected title");if(!t)throw new l("Expected subtitle");let a=t.nextElementSibling?.firstElementChild||null;if(!a)throw new l("Expected card link after subtitle");let[o,i]=C(a),[s,d]=P(n);return s.daaLl=`${r.textContent??""} - ${s?.daaLl}`,[{type:"Card",title:r.textContent??"",subtitle:t.textContent??"",bodyLink:o,footerCTA:s,eyeBrow:e.textContent??""},[...d,...i]]};var S={MissingBackgroundImageSection:"Promo card is missing background image section",MissingBackgroundImage:"Promo card is missing background image",MissingBackgroundImageAlt:"Promo card background image is missing alt text",MissingBackgroundImageSrc:"Promo card background image is missing src",MissingContentSection:"Promo card is missing card details section",MissingIcon:"Promo card is missing icon",MissingIconSrc:"Promo card icon is missing src",MissingIconAlt:"Promo card icon is missing alt text",MissingTitleElement:"Promo card is missing title element",MissingTitleText:"Promo card is missing title text",MissingSecondaryCtaAnchor:"Promo card is missing secondary CTA anchor"},jn=n=>{let[e,r]=n.querySelectorAll(":scope > div"),t=new Set;if(e===void 0)throw new l(S.MissingBackgroundImageSection);let a=e.querySelector(":scope picture:not(:scope p picture) img")??null;a===null&&t.add(new u(S.MissingBackgroundImage));let o=a?.getAttribute("alt")??"";o===""&&t.add(new u(S.MissingBackgroundImageAlt));let i=a?.getAttribute("src")??"";if(i===""&&t.add(new u(S.MissingBackgroundImageSrc)),r===void 0)throw new l(S.MissingContentSection);let s=r.querySelector('a[href$=".svg"]')??null;s===null&&t.add(new u(S.MissingIcon));let[d,c]=(s?.textContent?.split("|")??["",""]).map(h=>h.trim());d===""&&t.add(new u(S.MissingIconSrc)),c===""&&t.add(new u(S.MissingIconAlt));let x=r.querySelector("p > strong")??null;if(x===null)throw new l(S.MissingTitleElement);let y=x?.textContent??"";y===""&&t.add(new u(S.MissingTitleText)),r.querySelector("em > a")===null&&t.add(new u(S.MissingSecondaryCtaAnchor));let p="$29.90/mo",[f,v]=(()=>{try{return P(n)}catch{return[null,[]]}})();return v.forEach(h=>t.add(h)),f&&(f.daaLl=`${y} - ${f?.daaLl}`),[{type:"PromoCard",card:{bgImageAlt:o,bgImageSrc:i,iconAlt:c,iconSrc:d,title:y,cta:f,price:p}},[...t]]};var Fn=n=>{let e=new Set;if(n===null)throw new l(qn.elementNull);let r=n.querySelector("h2")?.textContent??"";r===""&&e.add(new u(qn.noTitle));let t=(async()=>{try{let a=n.querySelector("a"),o=new URL(a?.href??""),i=await H(o);if(i instanceof l)throw new Error(i.message);let s=await Sn(i);if(s instanceof l)throw new Error(s.message);return Tn(o.href,s),n.classList.contains("product-list")?Bn(s):Ge(s)}catch(a){throw new l(a?.message)}})();if(t instanceof l)throw t;return[{type:"MegaMenu",title:r,content:t},[...e]]},qn={elementNull:"Element is null",noTitle:"Large Menu has no Title"},Ge=n=>{let e=[...n.children];if(e.length===0)throw new l("No mega menu items found (did you forget to add them correctly?)");let[r,t]=M(e,a=>Ne(a));if(r.length===0)throw new l("Failed to parse gnav cards sections");return[{type:"GnavCards",sections:r},t]},Ne=n=>{let e=[...n.querySelectorAll(".featured-card, .links-card, .promo-card")];if(e.length===0)throw new l("Column contains no cards (did you forget to label them correctly?)");let[r,t]=M(e,a=>De(a));if(r.length===0)throw new l("Failed to parse cards in column");return[{type:"GnavColumn",cards:r},t]},De=n=>{if(n.classList.contains("featured-card"))return On(n);if(n.classList.contains("links-card"))return Dn(n);if(n.classList.contains("promo-card"))return jn(n);throw new l("Unsupported gnav cards section")};var Wn=({card:n})=>Be(n),Be=({title:n,subtitle:e,eyeBrow:r,footerCTA:t,bodyLink:a})=>`
...
`.trim();var Kn=({card:n})=>je(n),je=({bgImageAlt:n,bgImageSrc:e,iconAlt:r,iconSrc:t,title:a,cta:o,price:i})=>`
...
<nav>
  <ul>
    ${(()=>{let t=n.find(d=>d.type==="Brand")??null,a=n.filter(d=>d.type!=="Brand"),o=`
      ...
      `.trim(),i=t?xn(t):"",s=_n(a,xn);return`
